...
module.exports = function (filter, customOpts) {
  let allFiles = [];
  const options = _.merge(defaultOptions, customOpts);
  ...
  for (let dependency in dependencies) {
    let modulePath = j(options.nodeModules, dependency);
    ...
    const mainFile = dependencyPkg.main || '';
    const styleFile = dependencyPkg.style || '';
    ...
    if (!options.onlySpecified) {
      ...
      if (dependencyPkg.files) {
        dependencyPkg.files.map(e => depFiles[e] = true);
      }

      [mainFile, styleFile].forEach(file => {
